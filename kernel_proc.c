...
int procinfo_read(void *procinfocb, char* buf, unsigned int size){
  ...
  if(info->cursor > &PT[MAX_PROC-1])
    return 0;

  if(info->cursor->pstate != FREE){ 

      info->process_info.pid = get_pid(info->cursor);
      info->process_info.ppid = get_pid(info->cursor->parent);
      
      if(info->cursor->pstate == ALIVE)
        info->process_info.alive = 1;
      else
        info->process_info.alive = 0;

      info->process_info.thread_count = info->cursor->thread_count;
      info->process_info.main_task = info->cursor->main_task;
      info->process_info.argl = info->cursor->argl;
      
      if(info->cursor->args != NULL){
        for(int i = 0; i < info->process_info.argl; i++){
          info->process_info.args[i] = ((char *) info->cursor->args)[i];
        }
      }

      memcpy(buf, (char *)&info->process_info, sizeof(procinfo));
      info->cursor+=1;
...
Fid_t sys_OpenInfo()
{
  ...
  /* Create a new procinfo_control_block */
