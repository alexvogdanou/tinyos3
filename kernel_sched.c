...
TCB* spawn_thread(PCB* pcb, void (*func)())
{
	...
	/* Every thread at the beggining has maximum priority */
	tcb->priority = 0;
...
static void sched_queue_add(TCB* tcb)
{
	...
	/* insert the tcb on the right queue depending it's priority */
...
{
	/* find the first non empty scheduler queue */
	int sched_queue = 0;
	
	while(sched_queue < SCHED_QUEUES-1 && is_rlist_empty(&SCHED[sched_queue]))
		sched_queue++;

	rlnode* sel = rlist_pop_front(&SCHED[sched_queue]);
...
{
	//initialize our scheduler queues
